...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ url('admin/roleAdd') }}">角色列表</a></li>
                ...
                <form role="form" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" id="name" value="" placeholder="" autocomplete="off" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group" id="rules">
                            ...
                            @if (count($permissions) > 0)
                                @foreach ($permissions as $key=>$value)
                                    <div class="checkbox"><label><input type="checkbox" name="permission_id[]" value="{{ $value->id }}" data-id="{{ $value->id }}" data-parentid="{{ $value->parent_id }}">@if ($value->parent_id === 0) ｜ @endif {{ str_repeat('－',$value->level*4) }} {{ $value->title }}</label></div>
                                @endforeach
                            @else
