                ...
                <form role="form" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" id="name" value="{{ $rule->name }}" placeholder="" autocomplete="off" required>
                        </div>
                        <div class="form-group">
                            <label for="title">标题</label> @if ($errors->has('title')) {{ $errors->first('title') }} @endif
                            <input type="text" name="title" class="form-control" id="title" value="{{ $rule->title }}" placeholder="" autocomplete="off" required>
                        </div>
                        <div class="form-group">
                            <label for="icon">图标</label> @if ($errors->has('icon')) {{ $errors->first('icon') }} @endif
                            <input type="text" name="icon" class="form-control" id="icon" value="{{ $rule->icon }}" placeholder="" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="ismenu" class="form-control select2" style="width: 100%;">
                                <option value="1" @if ($rule->ismenu === 1) selected @endif>是</option>
                                <option value="0" @if ($rule->ismenu === 0) selected @endif>否</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="sort">排序</label> @if ($errors->has('sort')) {{ $errors->first('sort') }} @endif
                            <input type="text" name="sort" class="form-control" id="sort" value="{{ $rule->sort }}" placeholder="" required>
                        </div>
                        <div class="form-group">
