        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header with-border">
                    ...
                    <a href="{{ url('admin/addAdmin') }}" class="btn btn-sm btn-success" title="新增"><i class="fa fa-plus"></i><span class="hidden-xs"> 新增</span></a>
                </div>
                <!-- /.box-header -->
                        ...
                        @if (count($admins) > 0)
                            ...
                            @foreach ($admins as $key=>$value)
                                <tr>
                                    ...
                                    <td>@if (empty($value->avatar)) 暂无头像 @else <a href="/{{ $value->avatar }}" target="_blank">/{{ $value->avatar }}</a> @endif</td>
                                    <td>{{ $value->group->name }}</td>
                                    <td>{{ date('Y-m-d H:i:s',strtotime($value->created_at)) }}</td>
                                    <td>{{ date('Y-m-d H:i:s',strtotime($value->updated_at)) }}</td>
                                    <td>
                                        <a href="{{ url('admin/infoAdmin') }}?id={{ $value->id }}"><i class="fa fa-eye"></i></a>
                                        <a href="{{ url('admin/editAdmin') }}?id={{ $value->id }}"><i class="fa fa-edit"></i></a>
                                        <a href="javascript:void(0)" onclick="warning('确定要删除?','{{ url('admin/delAdmin') }}?id={{ $value->id }}')" class="grid-row-delete"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            </form>
