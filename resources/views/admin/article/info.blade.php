                ...
                <form role="form" method="post" enctype="multipart/form-data" onsubmit="if(!UM.getEditor('myEditor').hasContents()){alert('内容不能为空');return false;}">
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control" id="title" value="{{ $article->title }}" placeholder="" autocomplete="off" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control select2" style="width: 100%;">
                                @if (count($categorys) > 0)
                                    @foreach ($categorys as $key=>$value)
                                        <option value="{{ $value->id }}" @if ($article->category_id === $value->id) selected @endif> @if ($value->parent_id === 0) ｜ @endif {{ str_repeat('－',$value->level*4) }} {{ $value->name }}</option>
                                    @endforeach
                                @else
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="keywords" class="form-control" id="keywords" value="{{ $article->keywords }}" placeholder="" autocomplete="off">
                        </div>
                        <div class="form-group">
                            <label for="description">描述</label> @if ($errors->has('description')) {{ $errors->first('description') }} @endif
                            <textarea class="form-control" name="description" id="description" rows="3" placeholder="">{{ $article->description }}</textarea>
                        </div>
                        <div class="form-group" style="overflow: auto;">
                            <label for="content">内容</label> @if ($errors->has('content')) {{ $errors->first('content') }} @endif
                            <script type="text/plain" id="myEditor" name="content" style="width:100%;height:240px;">{!! $article->content !!}</script>
                        </div>
                    </div>
                    <!-- /.box-body -->
